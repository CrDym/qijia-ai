...
export function createAISettingsRepository(db: DatabaseSync) {
  return {
    read(): AISettingsInput | undefined {
      return db
        .prepare(
          "SELECT api_key AS apiKey, model, base_url AS baseURL FROM ai_settings WHERE id = 1",
        )
    ...
    save(input: AISettingsInput) {
      ...
      db.prepare(
        ...
        ON CONFLICT(id) DO UPDATE SET
          ...
          base_url = CASE WHEN ? THEN excluded.base_url ELSE ai_settings.base_url END`,
      ).run(
        input.apiKey,
        input.model,
        input.baseURL ?? "",
        input.baseURL === undefined ? 0 : 1,
      );
    },
    reset() {
