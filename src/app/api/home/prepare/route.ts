...
import { prepareHome } from "../../../../services/ai/prepare-home.ts";
  ...
  try {
    ...
    if (
      ...
      if (Array.from(form.keys()).some((key) => key !== "file"))
        throw new AppError("INVALID_UPLOAD", "请一次提供原文或一个文件");
      input = { file: singleFile(form)! };
    } else input = captureInputSchema.parse(await readJson(request));
    response = Response.json(await runExclusiveAI(() => prepareHome(input)));
  } catch (error) {
