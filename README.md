...
**提供内容，自动整理，确认后入库。** 资料库的“新增资料”入口在粘贴原文或上传文件/图片后自动调用已配置的 AI 服务，生成完整草稿，不必先选择分类或逐项采用建议；点击“确认保存”才会写入资料库。没有 AI Key 或整理失败时，可切换“手动填写”。已有记录编辑及家庭健康专用表单仍保留手动 AI 操作。
...
测试覆盖资料与成员管理、附件持久化、图片校验、自动分类草稿、确认前不入库、健康成员确认、删除保护和 AI 输出校验。测试不使用真实家庭资料，不调用付费 AI，也不代表对真实模型识别准确率的验证。
...
- “新增资料”会在提供内容后自动调用 AI，产生 API 用量：文字及文件提取的正文、或待识别图片会发送给当前配置的服务。手动表单只在点击 AI 按钮时发送；确认保存不会再次调用 AI。服务端设置 `store: false`，这不等同于供应商所有日志或留存政策的承诺。
